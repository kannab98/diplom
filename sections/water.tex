...
(\kappa_{nx}, \kappa_{ny})$ -- 
волновой вектор. В соответствии с
...
для моделирования поверхности для двух диапазонов излучения: $Ku$ и $C$.
...
\cite{pustovoytenko}. Как решить эту проблему обсудим в дальнейшем.
...
Следующая задача, которую надо решить, связяна с тем, как расположить гармоники по оси волновых чисел. 


Самый простой вариант расположения гармоник -- равномерный шаг, который можно определить следующим образом:
\begin{equation}
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{Корреляционная функция высот для равномерного распределения и 4-х
    скоростей ветра: (a) 5 м/с,  (b) 10 м/с, (c) 15 м/с и числе
гармоник $N=256$}
    \label{fig:corr_h_lin}
\end{figure}
Частично от артефактов можно избавиться, выбрав неравномерный шаг таким
образом, чтобы вблизи пика спектра $\kappa_m$ была большая плотность.

Для этого подойдет следующее распределение
\begin{equation}
    k_i = k_{min} + \frac{k_{cut} - k_{min}}{(N-1)^2} (i-1)^2
\end{equation}

Также можно попробовать вариант <<логарифмического>> шага для спектра
волнения. Тогда  положение гармоник вычисляются следующим образом
\begin{equation}
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{Корреляционная функция высот для логарифмического распределения и
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \caption{Корреляционная функция высот для неравномерного распределения и
    4-х скоростей ветра: (a) 5 м/с,  (b) 10 м/с, (c) 15 м/с и числе}
    \label{fig:corr_h_log}
\end{figure}

На рис.\ref{fig:corr_h_quad} и \ref{fig:corr_h_log} показаны корреляционные
функции волнения с разным числом гармоник в случае неравномерного выбора шага.
Логарифмический шаг -- на рис. \ref{fig:corr_h_quad} и неравномерный шаг на
рис. \ref{fig:corr_h_quad}.

Исходя из рис.\ref{fig:corr_h_lin},\ref{fig:corr_h_quad},\ref{fig:corr_h_log}
из предложенных предложенных методов всех лучше себя показывает логарифмическое
разбиение частотной области.



С увеличением скорости ветра число гармоник, необходимых для получения
одинакового качества моделирования, возрастает. На рис.\ref{fig:corr_h_log}
продемонстрирован этот эффект.  Хорошо заметно, что с ростом скорости ветра
$K[\rho]$   медленнее спадает к нулю, а значит $\tK[\rho]$ требует большего
количества синусоид для качественного моделирования.

Как показало тестовое моделирование, для получения <<качественной>> численной
реализации требуется большой число гармоник, что делает процесс вычислений
длительным. 
...
\begin{figure}[ht]
    ...
    \caption{ Корреляционная функция высот для логарифмического расположения
    гармоник (зеленая кривая) и расположения по методу отбеливания спектра
(синяя кривая) для скорости ветра 10 м/с}
\end{figure}
...
Современный центральные процессоры (CPU) уже давно имеют в своём распоряжении
...
На рисунке ниже приведено сравнение многопоточных вычислений на CPU и GPU из
 ...
 \begin{figure}[h!]
    ...
    \hfill
     \begin{subfigure}{0.49\linewidth}
        ...
        \caption{}
    \end{subfigure}
    \caption{(a) Плотность вероятности наклонов $W_{\xi_x}(z)$ для линейной
...
Для трехмерного случая Пирсон \cite{pierson} предоставил решение
